  ...
  try {
    for (let i = 0; i < cards.length; i++) {
      ...
      try {
        ...
        // both files — no review pause, and no confirmation prompt for
        // discarding the previous card's (already-exported) document.
...
nameLayer.textItem.contents=${q(card.Name)};

var cardCenterX=Number(template.width)/2;
var nameCentered=centerTextLayerHorizontally(nameLayer,cardCenterX);
app.echoToOE("NAMEDEBUG: contents="+${q(card.Name)}+" centeredBounds="+JSON.stringify(boundsPx(nameCentered.bounds))+" cardCenterX="+cardCenterX);
...
// Scale to fit the card's height (not full cover-fit), then center —
// do this immediately after paste, before any reordering, so resize()
// isn't racing an in-flight move() job.
var bb = boundsPx(newArt.bounds);
var w = bb[2] - bb[0];
var h = bb[3] - bb[1];
if (!(w > 0 && h > 0)) throw Error("Artwork layer has invalid bounds.");

var scale = (cardH / h) * 100; // percent — fit to card height
...
app.echoToOE("ARTDEBUG2: placed art, fit-height scale=" + scale +
             " cardW=" + cardW + " cardH=" + cardH);

// Hide the gray placeholder so only real art shows, and reorder newArt
// to sit just ABOVE it — done LAST, after all bounds reads/resizes, so
// nothing touches this layer's content after the reorder job fires.
// PLACEBEFORE = in front of / above the reference layer in the panel;
// PLACEAFTER = behind / below it — we want above, so PLACEBEFORE.
...
function cardStem(card) {
  return (
    String(card.CardNumber).padStart(3, "0") + " - " + card.Name
  ).replace(/[<>:"/\\|?*]/g, "_");
}
...
// directly in the Photopea panel) and exports + downloads PSD and PNG.
async function exportCard(index) {
  ...
  const psd = await exportCurrent("psd");
  const png = await exportCurrent("png");
  ...
  downloadBuffer(psd, `${stem}.psd`, "application/octet-stream");
  downloadBuffer(png, `${stem}.png`, "image/png");

  markCard(index, "done", "Exported PSD + PNG (including any manual edits).");
  renderCards();
}
